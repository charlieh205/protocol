...
1. Provide a simple way for engineers to have a look at standard network protocol headers, directly from the command-line, without having to google for the relevant RFC or for ugly header image diagrams.
2. Provide a way for researchers and engineers to quickly generate ASCII RFC-like header diagrams for their own custom protocols.

## Installation

The latest version of protocol is available via GitHub. It can be downloaded directly from the [repository](https://github.com/charlieh205/protocol) by clicking on the button "Download ZIP". Alternatively, the code can be checked out using `git`, via the following command.

```sh
git clone https://github.com/charlieh205/protocol.git
```

> If prompted for a username and password, don't enter any, just hit Enter.

Documentation and extra information is always available at this [site](http://www.luismg.com/protocol).
...
| Command-line identifier | Protocol |
| --- | --- |
| "ethernet" | Ethernet |
| "8021q" | IEEE 802.1q |
| "dot1q" | IEEE 802.1q |
...
> **Note:** protocol names don't need to be fully specified, as long as there is no ambiguity. For example "protocol et" is equivalent to "protocol ethernet". Whenever there is ambiguity (e.g. "protocol icmpv6-r", a list of possible options will be displayed.
...
| `startchar=<c>` | Instructs protocol to use the supplied character instead of the default "+" for the first position of a horizontal line. |
| `endchar=<c>` | Same as startchar but for the character in the last position of the horizontal lines. |
...
```txt
usage: protocol [-h] [-v] [-b BITS] [-n] [--even-char EVEN_CHAR] [--odd-char ODD_CHAR] [--start-char START_CHAR] [--end-char END_CHAR] [--sep-char SEP_CHAR] protocol [protocol ...]

positional arguments:
  protocol                the name of an existing protocol, or a field-by-field specification of a custom protocol

options:
  -h, --help              show this help message and exit
  -v, --version           show program's version number and exit
  -b BITS, --bits BITS    the number of bits per line
  -n, --no-numbers        do not print bit numbers on top of the header
  --even-char EVEN_CHAR   character for the even positions of horizontal table borders
  --odd-char ODD_CHAR     character for the odd positions of horizontal table borders
  --start-char START_CHAR character that starts horizontal table borders
  --end-char END_CHAR     character that ends horizontal table borders
  --sep-char SEP_CHAR     character that separates protocol fields
```
