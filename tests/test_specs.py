...
@pytest.mark.parametrize(
    ("spec", "expected"),
    [pytest.param(protocol.PROTOCOLS[spec], _EXPECTED[spec], id=spec) for spec in protocol.PROTOCOLS],
)
def test_specs(spec: str, expected: str) -> None:
    if expected == "":
        pytest.skip(f"Have not written test for '{spec}'")
    assert str(protocol.Protocol(spec)) == expected
